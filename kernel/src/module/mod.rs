...
use kernel_api::{elf::{elf64ehdr::Elf64Ehdr, elf64phdr::{Elf64Phdr, PF_R, PF_X, PT_LOAD}, elf64shdr::Elf64Shdr}, module::{Module, ModuleInfo, RawModule}};
use x86_64::structures::paging::{Mapper, Size4KiB};

use crate::{linker::Linker, module::error::LoadError};

mod error;

pub unsafe fn load_module(module: &RawModule, mapper: &mut impl Mapper<Size4KiB>) -> Result<Module, LoadError> {
    let file = unsafe { slice::from_raw_parts(module.raw_ptr as *const u8, module.raw_len as usize) };
...
fn is_in_executable_segment(module: &RawModule, addr: u64) -> bool {
    ...
    let ehdr = unsafe { &*(file.as_ptr() as *const Elf64Ehdr) };
    let phdrs = unsafe {
        slice::from_raw_parts(file.as_ptr().add(ehdr.e_phoff as usize) as *const Elf64Phdr, ehdr.e_phnum as usize)
    };
    phdrs.iter().any(|ph| {
        ph.p_type == PT_LOAD
            && ph.p_flags & PF_X != 0
...
pub unsafe fn read_module_info(module: &RawModule) -> Option<ModuleInfo> {
    ...
    let shdrs = unsafe {
        core::slice::from_raw_parts(module.base.add(ehdr.e_shoff) as *const Elf64Shdr, ehdr.e_shnum as usize)
    };
    
    let shstrtab = &shdrs[ehdr.e_shstrndx as usize];
    let strtab: &[u8] = unsafe {
        core::slice::from_raw_parts((module.base as *const u8).add(shstrtab.sh_offset as usize), shstrtab.sh_size as usize)
    };

    for shdr in shdrs {
        let start_name = shdr.sh_name as usize;
        let end_name = strtab[start_name..].iter().position(|&b| b == 0).map(|x| start_name + x).unwrap_or(strtab.len());
        let name = &strtab[start_name..end_name];
        if name == b".module_info" {
            let module_info_ptr = unsafe {
                (module.base as *const u8).add(shdr.sh_offset as usize)
            };
